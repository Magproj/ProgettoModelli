...
function merchant_account_id() {

	global $CI;

	return $CI->mdl_mcb_data->setting('merchant_account_id');
	
}

function merchant_currency_code() {

	global $CI;

	return $CI->mdl_mcb_data->setting('merchant_currency_code');
	
}

function merchant_driver() {

	global $CI;

	return $CI->mdl_mcb_data->setting('merchant_driver');
...
function invoice_final_payment_date($invoice) {

	global $CI;

	/* Date the invoice was paid in full */
	krsort($invoice->invoice_payments);
	return date($CI->mdl_mcb_data->setting('default_date_format'), $invoice->invoice_payments['0']->payment_date);
...
function invoice_logo($output_type = 'pdf') {
	...
	if ($CI->mdl_mcb_data->setting('include_logo_on_invoice') == 'TRUE' AND $CI->mdl_mcb_data->setting('invoice_logo')) {
		...
		if ($output_type == 'pdf') {
			...
			return '<img src="' . getcwd() . '/uploads/invoice_logos/' . $CI->mdl_mcb_data->setting('invoice_logo') . '" alt="'.$CI->mdl_mcb_data->setting('invoice_logo').'" />';
		...
		elseif ($output_type == 'html') {
			...
			return '<img src="' . base_url() . 'uploads/invoice_logos/' . $CI->mdl_mcb_data->setting('invoice_logo') . '" alt="'.$CI->mdl_mcb_data->setting('invoice_logo').'" />';
