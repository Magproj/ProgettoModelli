...
public class BalancingController extends Controller {
	...
	*/
	 ...
	 * @throws InvalidValueException
	 */
	private Tupel<Long> calculateMinMaxValues(Ess essV, double percentage, double cosPhi, int phase)
			throws InvalidValueException {
		long maxPower = 0;
		long minPower = 0;
		percentage = Math.abs(percentage);

		Optional<Long> writeMin = essV.getSetActivePower(phase).writeMin();
		Optional<Long> writeMax = essV.getSetActivePower(phase).writeMax();
		...
		if (ControllerUtils.calculateApparentPower(minPower, cosPhi) > essV.allowedApparent.value() / 3) {
			minPower = ControllerUtils.calculateActivePowerFromApparentPower(essV.allowedApparent.value() / 3, cosPhi)
					* -1;
		}
		if (ControllerUtils.calculateApparentPower(maxPower, cosPhi) > essV.allowedApparent.value() / 3) {
			maxPower = ControllerUtils.calculateActivePowerFromApparentPower(essV.allowedApparent.value() / 3, cosPhi);
		}
