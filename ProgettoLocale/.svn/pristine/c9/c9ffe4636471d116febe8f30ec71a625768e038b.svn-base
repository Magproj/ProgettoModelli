...
public class WriteChannel<T> extends ReadChannel<T> {
	...
	public T checkMin(T min, T wMin){

		if (min == null || (wMin != null && wMin instanceof Comparable
				&& ((Comparable<T>) wMin).compareTo(min) > 0)) {
			min = wMin;
		}
	...
	public T checkMax(T max, T wMax){

		if (max == null || (wMax != null && wMax instanceof Comparable
				&& ((Comparable<T>) wMax).compareTo(max) < 0)) {
			max = wMax;
		}
